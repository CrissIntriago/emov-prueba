...
public class ServidorService extends AbstractService<Servidor> {
    ...
    private static final Logger LOG = Logger.getLogger(ServidorService.class.getName());
    ...
    public Servidor findByIdentificacion(String identificacion, Date inicio, Date fin) {
        try {
            return this.findByNamedQuery1("Servidor.findByPersonaIdentificacionRol", identificacion, inicio, fin);
        } catch (Exception e) {
            LOG.log(Level.SEVERE, identificacion, e);
        }
        return null;
    ...
    public Cliente findByServidorByIdentificacion(String identificacion) {
        Cliente cliente = (Cliente) em.createQuery("SELECT p FROM Servidor s INNER JOIN s.persona p WHERE p.identificacion=?1 AND s.estado=TRUE")
                .setParameter(1, identificacion)
                .getResultStream().findFirst().orElse(null);
        return cliente;
    }

    public Cliente findByProveedorByIdentificacionCompleta(String identificacionCompleta) {
        if (identificacionCompleta.length() > 10) {
            Cliente cliente = (Cliente) em.createQuery("SELECT c FROM Proveedor p JOIN p.cliente c WHERE c.identificacion = ?1 AND c.ruc= ?2  AND p.estado = TRUE")
                    .setParameter(1, identificacionCompleta.substring(0, 10))
                    .setParameter(2, identificacionCompleta.substring(10, 13))
                    .getResultStream().findFirst().orElse(null);
            return cliente;
        } else {
            Cliente cliente = (Cliente) em.createQuery("SELECT c FROM Proveedor p JOIN p.cliente c WHERE c.identificacion = ?1 AND p.estado = TRUE")
                    .setParameter(1, identificacionCompleta)
                    .getResultStream().findFirst().orElse(null);
            return cliente;
        }
    }
...
//        try {
//            Servidor s;
//            Query query = em.createQuery("SELECT s From Servidor s JOIN s.distributivo d JOIN d.unidadAdministrativa u JOIN u.tipoUnidad cu WHERE u.id = ?1 AND s.estado = TRUE AND d.cargo.nombreCargo LIKE CASE WHEN cu.codigo = 'DIR' THEN 'DIRECTOR%' WHEN cu.codigo = 'DE' THEN 'JEFE%' END  ")
...
//    }
    ...
    public Servidor findServidorByUnidad(UnidadAdministrativa unid) {
        try {
            Query query = em.createQuery("SELECT th.idServidor From  ThServidorCargo th  JOIN th.idCargo c JOIN th.idServidor s WHERE c.idUnidad = :unidad and th.activo = true and s.estado = true")
                    .setParameter("unidad", unid);
            return (Servidor) query.getResultStream().findFirst().orElse(null);
        } catch (NoResultException e) {
            return null;
        }
    }
