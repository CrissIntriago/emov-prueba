 ...
 * @author ORIGAMIEC
 */
@Named(value = "anulacionReservaView")
@ViewScoped
public class AnualcionReservaController extends BpmnBaseRoot implements Serializable {
    ...
    private static final Logger LOG = Logger.getLogger(AnualcionReservaController.class.getName());
...
//        }
    ...
    public void eliminarSolicitudCompleta() {
        ...
        List<Presupuesto> lis = reservaCompromisoService.listaPresupuestoPDI(reservaCompromiso.getPeriodo());
        if (!lis.isEmpty()) {
    ...
    public void abriDlogoAnulacion(SolicitudReservaCompromiso s) {
        reservaCompromiso = new SolicitudReservaCompromiso();
        reservaCompromiso = s;

        if ("ADA".equals(reservaCompromiso.getEstado().getCodigo()) || "LIQUI".equals(reservaCompromiso.getEstado().getCodigo())) {
            JsfUtil.addWarningMessage("AVISO", "NO PUEDE ANULAR ESTA RESERVA PORQUE SE ENCUENTRA " + reservaCompromiso.getEstado().getTexto());
            return;
        }
    ...
    public void actualzandoReservaPartidasDistAndDa(Short periodo) {
        ...
        if (lis != null && !lis.isEmpty()) {
            for (ThCargoRubros li : lis) {
                ...
                reservaCompromisoService.actualizarReservaPresupuesto(valor2, periodo, li.getId());
        ...
        if (tmpPd != null && !tmpPd.isEmpty()) {
            for (ProformaPresupuestoPlanificado li : tmpPd) {
                ...
                reservaCompromisoService.actualizarReservaPresupuestoPd(valor2, periodo, li.getId());
        ...
        if (!productoReservaAnuladas.isEmpty()) {
            ...
            for (Producto data : productoReservaAnuladas) {
                ...
                reservaCompromisoService.updateReservaProducto(num2, reservaCompromiso.getPeriodo(), data.getId());
