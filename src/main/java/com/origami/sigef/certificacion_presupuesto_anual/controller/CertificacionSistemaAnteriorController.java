 ...
 * @author ENRIQUE
 */
@Named(value = "certificacionSistemaAnteriorView")
@ViewScoped
public class CertificacionSistemaAnteriorController implements Serializable {
    ...
    private BigDecimal totalDevengado, totalDevengadoPD;
    private BigDecimal totalAdevengar;
    private List<Cliente> listaBeneficiarios;
    private int conteo, tabindex;
    ...
    private BigDecimal valorDevengar = BigDecimal.ZERO;
    private BigDecimal totalDevengadoPDI = BigDecimal.ZERO;
    private Date certFechaAprob;
    ...
    public void inicializar() {
        ...
        reservaCompromiso.setAntiguo(true);
        certFechaAprob = new Date();
    ...
    public void seleccionPresupuesto(ThCargoRubros p) {
        ...
        PrimeFaces.current().ajax().update("datosEnDetalle");
        JsfUtil.addInformationMessage("", "SELECCIONADO CON ÉXITO");
    }
    ...
    public void sumaTotales() {
        ...
        totalAdevengar = BigDecimal.ZERO;
        totalDevengadoPD = BigDecimal.ZERO;
        totalDevengadoPDI = BigDecimal.ZERO;
        ...
        for (DetalleSolicitudCompromiso detalle : listaGlobalDetalleSolicitudPartidas) {
            ...
            totalAdevengar = totalAdevengar.add(detalle.getMontoDevengar());
            totalDevengadoPD = totalDevengadoPD.add(detalle.getMontoDevengar());
        }
        for (DetalleSolicitudCompromiso detalle : listaGlobalDetalleSolicPartidasDirectas) {
            ...
            totalAdevengar = totalAdevengar.add(detalle.getMontoDevengar());
            totalDevengadoPDI = totalDevengadoPDI.add(detalle.getMontoDevengar());
        }
        if (listaGlobalDetalleSolicPartidasDirectas != null) {
            for (DetalleSolicitudCompromiso detalle : listaGlobalDetalleSolicPartidasDirectas) {
                totalDevengado = totalDevengado.add(detalle.getMontoDevengar());
                totalDevengadoPDI = totalDevengadoPDI.add(detalle.getMontoDevengar());
            }
        }
    }
    ...
    public void actualizarListadoReservas() {
        if (opcionBusqueda.getAnio() != null) {
            ...
            lazyReal.getFilterss().put("antiguo", true);
            //.getFilterss().put("usuarioCreacion:equal", user.getNameUser());
    ...
    public void savesolicitud() {
        ...
        if (listaGlobalDetalleSolicitudCompromisos != null) {
            for (DetalleSolicitudCompromiso itemsuma : listaGlobalDetalleSolicitudCompromisos) {
                ...
                if (itemsuma.getMontoDevengar() == null) {
                    JsfUtil.addWarningMessage("INCONSISTENCIA", "ASIGNAR MONTO DEVENGAR EN EL PAPP ANTES DE GUARDAR");
                    return;
                } else {
                    result1 = num1 + itemsuma.getMontoDevengar().doubleValue();
                }
                if (result1 > itemsuma.getActividadProducto().getMontoReformada().doubleValue()) {
                    verificar = false;
                    PrimeFaces.current().ajax().update("messages");
                    JsfUtil.addErrorMessage("Información", "Este producto " + itemsuma.getActividadProducto().getDescripcion() + " excede el monto disponible");
                    break;
                }
            }
        }
        ...
        reservaCompromiso.setAntiguo(Boolean.TRUE);
        ...
        for (DetalleSolicitudCompromiso li : listaGuardar) {
            ...
            detalleSolicitud.setMontoDevengar(li.getMontoDevengar());
    ...
    public BigDecimal obtTotaleDevengado(Integer tipo) {
        BigDecimal total = BigDecimal.ZERO;
        if (tipo == 1) {
            if (listaGlobalDetalleSolicitudCompromisos != null) {
                for (DetalleSolicitudCompromiso detalle : listaGlobalDetalleSolicitudCompromisos) {
                    total = total.add(detalle.getMontoDevengar());
                }
            }
        } else if (tipo == 2) {
            if (listaGlobalDetalleSolicitudPartidas != null) {
    ...
    public void solicitarReservas(DetalleSolicitudCompromiso d) {
        detalleSolicitud = new DetalleSolicitudCompromiso();
        detalleSolicitud = d;
        System.out.println("valorDevengar "+valorDevengar);
        if (valorDevengar == BigDecimal.ZERO) {
            detalleSolicitud.setMontoDevengar(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("tabPAPP");
        }
        if (valorDevengar.doubleValue() > detalleSolicitud.getMontoDisponible().doubleValue()) {
            detalleSolicitud.setMontoDevengar(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addErrorMessage("Información", "El monto a devengar es mayor al monto disponible");
        } else {
            detalleSolicitud.setMontoDevengar(valorDevengar);
    ...
    public void solicitarReservasPartidas(DetalleSolicitudCompromiso d) {
        detalleSolicitud = new DetalleSolicitudCompromiso();
        detalleSolicitud = d;
        if (detalleSolicitud.getMontoDevengar() == null) {
            detalleSolicitud.setMontoDevengar(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("tabPD");
        }
        if (detalleSolicitud.getMontoDevengar().doubleValue() > detalleSolicitud.getMontoDisponible().doubleValue()) {
            detalleSolicitud.setMontoSolicitado(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addErrorMessage("Información", "El monto a devengar es mayor al monto disponible");
        } else {
        ...
        calcularTotalesPD();
        PrimeFaces.current().ajax().update("tablaSolicitudPdi","tablaSolicitud2");
    }
    ...
    public void solicitarReservasPD(DetalleSolicitudCompromiso d) {
        ...
        if (detalleSolicitud.getMontoDevengar() == null) {
            detalleSolicitud.setMontoDevengar(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("tabPartidasD");
        }
        if (detalleSolicitud.getMontoDevengar().doubleValue() > detalleSolicitud.getMontoDisponible().doubleValue()) {
            detalleSolicitud.setMontoDevengar(BigDecimal.ZERO);
            PrimeFaces.current().ajax().update("messages");
        ...
        calcularTotalesPD();
        sumaTotales();
        JsfUtil.update("tablaSolicitudPdi");
        JsfUtil.update("tablaSolicitud2");
    }
...
//        if (s.getEstado().getCodigo().equals("APRO")) {
...
//        }
        ...
        if (reservaCompromiso.getBeneficiario() != null) {
            ...
            List<BeneficiarioSolicitudReserva> ver = reservaBeneficiarioService.getListaBeneficiarioSolicitudReservas(s);
            for (BeneficiarioSolicitudReserva data : ver) {
    ...
    public void editarReservaCompromiso() {
        ...
        if (!listaGlobalDetalleSolicitudCompromisos.isEmpty() || !listaGlobalDetalleSolicitudPartidas.isEmpty() || !listaGlobalDetalleSolicPartidasDirectas.isEmpty()) {
            if (listaGlobalDetalleSolicitudCompromisos != null && !listaGlobalDetalleSolicitudCompromisos.isEmpty()) {
                listaGlobalDetalleSolicitudCompromisos.forEach((l) -> {
                    System.out.println("l "+l.getMontoDevengar());
                    listaGuardar.add(l);
                });
            }
            ...
            if (li.getId() != null) {
                ...
                for (DetalleSolicitudCompromiso lli : listaValoresAnteriores) {
                    if (lli.equals(li)) {
                        ...
                        detalleSolicitud.setMontoDevengar(lli.getMontoDevengar());
                    }
                }
    ...
    public void aprobarSolicitudInmedi(SolicitudReservaCompromiso s) {
        this.reservaCompromiso = s;
        reservaCompromiso.setEstado(reservaCompromisoService.getestados("APRO", "estado_solicitud"));
        reservaCompromiso.setFechaActualizacion(new Date());
        reservaCompromiso.setFechaModificacion(new Date());
        reservaCompromiso.setUsuarioModificacion(userSession.getNameUser());
        if (reservaCompromiso.getFechaAprobacion() == null) {
            reservaCompromiso.setFechaAprobacion(new Date());
        }
        reservaCompromiso.setComprometido(Boolean.TRUE);
        reservaCompromisoService.edit(reservaCompromiso);
        PrimeFaces.current().ajax().update("messages");
        JsfUtil.addSuccessMessage("Información", "La solicitud " + s.getSecuancialForFilter() + " se Aprobó correctamente");
    }

    public void updateFechaApro(SolicitudReservaCompromiso item) {
        reservaCompromiso = item;
        reservaCompromiso.setFechaAprobacion(certFechaAprob);
    }
    ...
    public BigDecimal getValorDevengar() {
        return valorDevengar;
    }

    public void setValorDevengar(BigDecimal valorDevengar) {
        this.valorDevengar = valorDevengar;
    }
    
    public Date getCertFechaAprob() {
        return certFechaAprob;
    }

    public void setCertFechaAprob(Date certFechaAprob) {
        this.certFechaAprob = certFechaAprob;
    }
    ...
    public BigDecimal getTotalAdevengar() {
        return totalAdevengar;
    }

    public void setTotalAdevengar(BigDecimal totalAdevengar) {
        this.totalAdevengar = totalAdevengar;
    }
    ...
    public BigDecimal getTotalDevengadoPD() {
        return totalDevengadoPD;
    }

    public void setTotalDevengadoPD(BigDecimal totalDevengadoPD) {
        this.totalDevengadoPD = totalDevengadoPD;
    }

    public BigDecimal getTotalDevengadoPDI() {
        return totalDevengadoPDI;
    }

    public void setTotalDevengadoPDI(BigDecimal totalDevengadoPDI) {
        this.totalDevengadoPDI = totalDevengadoPDI;
    }
