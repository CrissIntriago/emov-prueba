 ...
 * @author ORIGAMIEC
 */
@Named(value = "anulacionReservaSinProcesoView")
@ViewScoped
public class AnulacionReservaSinProcesoController implements Serializable {
    ...
    private boolean renderedSinRequisitos;
    private List<CatalogoItem> estadoFiltro;
    private List<UnidadAdministrativa> unidadFiltros;
    private String observaciones, observacionAnulacion;
    private static final Logger LOG = Logger.getLogger(AnulacionReservaSinProcesoController.class.getName());
...
//        }
    ...
    public void actualizarListadoReservas() {
        if (opcionBusqueda.getAnio() != null) {
            ...
            if (!user.getNameUser().equals(clienteService.getrolsUser(RolUsuario.financiero))) {
    ...
    public void eliminarSolicitudCompleta() {

        List<DetalleSolicitudCompromiso> dataList = reservaCompromisoService.getListaDetlleSolciitud(reservaCompromiso);

        if (dataList.isEmpty()) {
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addWarningMessage("Advertencia", "No hay datos en esta reserva");
            return;
        }

        for (DetalleSolicitudCompromiso data : dataList) {
            if (data.getMontoComprometido().doubleValue() > 0) {
                PrimeFaces.current().ajax().update("messages");
                JsfUtil.addWarningMessage("Advertencia", "No se puede eliminar porque tiene valores Comprometidos");
                return;
            }

        }

        reservaCompromiso.setEstado(reservaCompromisoService.getestados("ADA", "estado_solicitud"));
        reservaCompromisoService.edit(reservaCompromiso);
        ...
        List<Producto> listP = reservaCompromisoService.listaReservasSinRepetirProducto(reservaCompromiso.getPeriodo());

        if (!listP.isEmpty()) {
            for (Producto pr : listP) {

                valor = reservaCompromisoService.sumaTotalDeReservasProducto(reservaCompromiso.getPeriodo(), pr.getId());

                result = reservaCompromisoService.updateReservaProducto(valor, reservaCompromiso.getPeriodo(), pr.getId());
        ...
        List<ThCargoRubros> lis = reservaCompromisoService.listaReservasSinRepetir(reservaCompromiso.getPeriodo());

        if (!lis.isEmpty()) {

            for (ThCargoRubros li : lis) {

                valor2 = reservaCompromisoService.sumaTotalDeReservasDevengado(reservaCompromiso.getPeriodo(), li.getId());
                result2 = reservaCompromisoService.actualizarReservaPresupuesto(valor2, reservaCompromiso.getPeriodo(), li.getId());
        ...
        List<ProformaPresupuestoPlanificado> tmpPd = reservaCompromisoService.listaReservasPdSinRepetir(reservaCompromiso.getPeriodo());
        if (tmpPd != null && !tmpPd.isEmpty()) {
            for (ProformaPresupuestoPlanificado li : tmpPd) {
                valor3 = reservaCompromisoService.sumaTotalDeReservasDevengadoPd(reservaCompromiso.getPeriodo(), li.getId());
                result3 = reservaCompromisoService.actualizarReservaPresupuestoPd(valor2, reservaCompromiso.getPeriodo(), li.getId());
                if (result3 > 0) {
        ...
        // RESERVAS ANUJLADAS ACTUALIZAR EL VALOR
        List<Producto> productoReservaAnuladas = reservaCompromisoService.listaReservasSinRepetirProductoAnulados(reservaCompromiso.getPeriodo());
        if (!productoReservaAnuladas.isEmpty()) {

            for (Producto data : productoReservaAnuladas) {
                BigDecimal num1 = BigDecimal.ZERO, num2 = BigDecimal.ZERO;
                num1 = reservaCompromisoService.sumaTotalDeReservasProductoAnulados(reservaCompromiso.getPeriodo(), data.getId());
                num2 = data.getReserva().subtract(num1);
                if (num2.doubleValue() < 0) {
                    num2 = BigDecimal.ZERO;
                }

                result = reservaCompromisoService.updateReservaProducto(num2, reservaCompromiso.getPeriodo(), data.getId());
        ...
        //sumaTotalDeReservasProductoAnulados
        PrimeFaces.current().executeScript("PF('dlogComenatio').hide()");
        PrimeFaces.current().ajax().update(":formComentario");

    }
    
    
    public void motivoAnulacion (SolicitudReservaCompromiso s) {
        reservaCompromiso = s;
        PrimeFaces.current().executeScript("PF('dlgmotivoAnulacion').show()");
    }

    public void abriDlogoAnulacion() {
        List<DetalleSolicitudCompromiso> dataList = reservaCompromisoService.getListaDetlleSolciitud(reservaCompromiso);
        if (dataList.isEmpty()) {
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addWarningMessage("Advertencia", "No hay datos en esta reserva");
        ...
        for (DetalleSolicitudCompromiso data : dataList) {
            if (data.getEjecutado().doubleValue() > 0) {
                PrimeFaces.current().ajax().update("messages");
                JsfUtil.addWarningMessage("Advertencia", "No se puede eliminar porque tiene valores Devengados");
                return;
            }
        }
        for (DetalleSolicitudCompromiso data : dataList) {
        ...
        reservaCompromiso.setEstado(reservaCompromisoService.getestados("ADA", "estado_solicitud"));
        reservaCompromiso.setObservacionAnulacion(observacionAnulacion);
        reservaCompromisoService.edit(reservaCompromiso);
        BigDecimal valor = BigDecimal.ZERO;
        BigDecimal valor2 = BigDecimal.ZERO;
        int result = 0;
        ...
        actualzandoReservaPartidasDistAndDa(reservaCompromiso.getPeriodo());
        List<Presupuesto> lispdi = reservaCompromisoService.listaPresupuestoPDI(reservaCompromiso.getPeriodo());
        if (!lispdi.isEmpty()) {
            for (Presupuesto li : lispdi) {
                valor2 = reservaCompromisoService.sumaTotalDeReservasDevengado(reservaCompromiso.getPeriodo(), li.getId());
                reservaCompromisoService.updatePDIReservas(valor2, reservaCompromiso.getPeriodo(), li);
                if (result2 > 0) {
                }
            }
        }
        ...
        PrimeFaces.current().ajax().update("messages");
        JsfUtil.addSuccessMessage("Información", "La solicitud se anulo correctamente");
        PrimeFaces.current().executeScript("PF('dlgmotivoAnulacion').hide()");
        PrimeFaces.current().ajax().update("dataSolciitudesReserva");
    }
    ...
    public void visualizarDetalleSolcitud(SolicitudReservaCompromiso s) {
        ...
        if (s.getProcedimiento() == null && s.getNumTramite() == null) {
            renderedSinRequisitos = false;
        }
    ...
    public void handleFileUpload(FileUploadEvent event) throws IOException {
        ...
        try {
            ...
            fos.write(contents);
            fos.close();
            solicitudRequisito.setUrl(uploadFile.getFileName());
            solicitudRequisito.setIdProcedimientoRequisito(procedimientoRequisito);
            solicitudRequisitoList.add(solicitudRequisito);
    ...
    public void completarTareaAnulacion() {
    ...
    public void actualzandoReservaPartidasDistAndDa(Short periodo) {
        ...
        if (!listP.isEmpty()) {
            for (Producto pr : listP) {
                valor1 = reservaCompromisoService.sumaTotalDeReservasProducto(periodo, pr.getId());
                int result = reservaCompromisoService.updateReservaProducto(valor1, periodo, pr.getId());
                if (result > 0) {
                }
            }
        }
        ...
        if (tmpPd != null && !tmpPd.isEmpty()) {
            for (ProformaPresupuestoPlanificado li : tmpPd) {
                valor3 = reservaCompromisoService.sumaTotalDeReservasDevengadoPd(periodo, li.getId());
                int result3 = reservaCompromisoService.actualizarReservaPresupuestoPd(valor2, periodo, li.getId());
    ...
    public boolean isRenderedSinRequisitos() {
        return renderedSinRequisitos;
    }

    public void setRenderedSinRequisitos(boolean renderedSinRequisitos) {
        this.renderedSinRequisitos = renderedSinRequisitos;
    }
