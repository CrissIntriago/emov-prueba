 ...
 */
@Named(value = "liberacionReservaView")
@ViewScoped
public class LiberacionMB extends BpmnBaseRoot implements Serializable {
    ...
    public void liberar() {
        ...
        for (DetalleSolicitudCompromiso item : listaSolicitudes) {
            item.setLiberado(item.getMontoSolicitado().subtract(item.getMontoComprometido()));
            item.setMontoSolicitado(item.getMontoSolicitado().subtract(item.getLiberado()));
        }
    ...
    public void liquidadReserva() {
        ...
        try {
            ...
            estadoLiquidado = catalogoService.getItemByCatalogoAndCodigo("estado_solicitud", "LIBE");
            ...
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addInformationMessage("INFORMACIÓN", "LA SOLICITUD SE HA LIBERADO CORRECTAMENTE Y HA SIDO ENVIADA CORRECTAMENTE");
