...
@Named
@ViewScoped
public class FirmaElectronicaMB extends BpmnBaseRoot implements Serializable {
    ...
    public void completarTarea(SolicitudReservaCompromiso s) {
        try {
            ...
            if (clienteNotifacacion != null) {
                if (clienteNotifacacion.getEmail() != null) {
                    switch (tramite.getTipoTramite().getActivitykey()) {
                        case "solicitud_reserva_compromiso":
                            ...
                            break;
                        case "modificacionReservaComprimosProccess":
                            enviarCorreo(clienteNotifacacion.getEmail(), "Solicitud Reserva Compromiso", formatoCodigo(s.getSecuencial()), clienteNotifacacion.getNombre() + " " + clienteNotifacacion.getApellido());
                            break;
                        case "anulacionReservaCompromiso":
                            enviarCorreo(clienteNotifacacion.getEmail(), "Solicitud Reserva Compromiso", formatoCodigo(s.getSecuencial()), clienteNotifacacion.getNombre() + " " + clienteNotifacacion.getApellido());
                            break;
                        case "liquidacionReservaCompromisoProcess":
                            enviarCorreo(clienteNotifacacion.getEmail(), "Solicitud Reserva Compromiso", formatoCodigo(s.getSecuencial()), clienteNotifacacion.getNombre() + " " + clienteNotifacacion.getApellido());
                            break;
                        default:
            ...
            PrimeFaces.current().ajax().update("messages");
            JsfUtil.addInformationMessage("INFORMACIÓN", "LA TAREA HA FINALIZADO CORRECTAMENTE");
            JsfUtil.redirectFaces("/procesos/bandeja-tareas");
        } catch (Exception e) {
    ...
    public void armarDocumento(SolicitudReservaCompromiso s) {
        ...
        try {
            ...
            if ("documento_l".equals(tarea.getTaskDefinitionKey())) {
...
//        }
    ...
    public void descargarDocumento(SolicitudReservaCompromiso s) {
        switch (tramite.getTipoTramite().getActivitykey()) {
            case "solicitud_reserva_compromiso":
                ss.setNombreDocumento(s.getRutaReserva());
                break;
            case "modificacionReservaComprimosProccess":
                ss.setNombreDocumento(s.getRutaModificada());
                break;
            case "anulacionReservaCompromiso":
                ss.setNombreDocumento(s.getRutaAnulada());
                break;
            case "liquidacionReservaCompromisoProcess":
                ss.setNombreDocumento(s.getRutaLiquidada());
                break;
            case "liberacionReservaCompromiso":
                ss.setNombreDocumento(s.getRutaLiberada());
                break;
        }
