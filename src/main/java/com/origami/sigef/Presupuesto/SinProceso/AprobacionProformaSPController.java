...
import com.origami.sigef.contabilidad.controller.PresupuestoController;
 ...
 * @author Luis Suarez
 */
@Named(value = "arpoabcionProformaspView")
@ViewScoped
public class AprobacionProformaSPController implements Serializable {
    ...
    public void clearAllFilters() {
        ...
        DataTable dataTable = (DataTable) FacesContext.getCurrentInstance().getViewRoot().findComponent("formMain:catalogoProformaPresupuestoTable");
        if (!dataTable.getFilters().isEmpty()) {
            dataTable.reset();

            PrimeFaces.current().ajax().update("formMain:catalogoProformaPresupuestoTable");
        }
    }
...
//                        PrimeFaces.current().executeScript("PF('confirmarAprobado').show()");
            ...
            if (!mantenimientoProformaService.validaCodigoPresupuestario()) { // NO PERMITIR APROBAR PRESUPUESTO CON PRODUCTOS SIN PARTIDAS
                JsfUtil.addErrorMessage("Aviso", "Existen productos sin Código Presupuestario Asignado");
                return;
            }
    ...
    public void save() {
        if (mantenimientoProforma.getAprobado() == true) {
            if (mantenimientoProforma.getTipoFlujo() == true) {
                ...
                actividadService.aprobarTodo(mantenimientoProforma.getPeriodo(), estadoAprobadoPapp);
                actividadService.aprobarDistributivosAndPD(mantenimientoProforma.getPeriodo(), estadoAprobadoOtros);
            }
        }
...
//        PrimeFaces.current().executeScript("PF('confirmarAprobado').hide()");
    ...
    public void rechazado(CatalogoProformaPresupuesto c) {
        if (!c.getTipoFlujo()) {
            ...
            actividadService.aprobarTodo(c.getPeriodo(), estadoRecahazadoPapp);
            actividadService.aprobarDistributivosAndPD(c.getPeriodo(), estadoRecahazadoOtros);
        }
    }
...
//            PrimeFaces.current().executeScript("PF('confirmarAprobado').hide()");
    ...
    public String redireccionar(CatalogoProformaPresupuesto catalogo) {
        ...
        if (catalogo.getTipoFlujo()) {
            return "proformaIngresoSinProceso.xhtml?faces-redirect=true";
        } else {
            return "proformaEgresosSinProceso.xhtml?faces-redirect=true";
        }
    }
