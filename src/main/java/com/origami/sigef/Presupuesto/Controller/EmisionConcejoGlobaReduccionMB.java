...
@Named
@ViewScoped
public class EmisionConcejoGlobaReduccionMB extends BpmnBaseRoot implements Serializable {
...
//<editor-fold defaultstate="collapsed" desc="AGRUPAR DISTRIBUTIVO">
        if (listaagrupacionDistr != null && !listaagrupacionDistr.isEmpty()) {
            ...
            for (ProformaPDTO i : listaagrupacionDistr) {
                ...
                proformaPresupuesto.setUsuarioModificacion(userSession.getNameUser());
                proformaPresupuesto.setFechaModificacion(new Date());
                proformaPresupuesto.setCodigo(proformaPresupuesto.getCodigo());
...
//</editor-fold>
    ...
    public void abriDlogo(ReformaIngresoSuplemento r, boolean aprobar) {
        ...
        if (aprobar) {
            ...
            btnAprobar = true;
            btnRechazar = false;
//            List<Producto> productosPartidas = reformaService.showProductoNuevo(BigInteger.valueOf(reforma.getId()));
    ...
    public void handleFileUpload(FileUploadEvent event) {
        ...
        Thread data = new Thread() {
            ...
            public void run() {
                ...
                uploadDoc.upload(reforma, files);
            }
        };

        data.start();
        JsfUtil.addInformationMessage("", "EL archivo se subió correctamente");
    }
