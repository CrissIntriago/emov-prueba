...
@Named
@ViewScoped
public class BancoProyectoMB implements Serializable {
    ...
    public void importarExcel(FileUploadEvent event) {
        try {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (next > 0) {
                    if (row != null && row.getCell(0) != null && row.getCell(1).getStringCellValue() != null) {
...
//                        }
                        ...
                        listModelo.add(new PlanLocalFormat(codSistema, row.getCell(1).getStringCellValue(), codObjetivo, row.getCell(3).getStringCellValue(), codPolitica, row.getCell(5).getStringCellValue()));
                    }
                }
                next += 1;
            }
...
//            }
//            for (PlanLocalObjetivo pl: listaObjetivos) {
//                //planSistemaService.create(l);
//                System.out.println(pl.getDescripcion());
//            }
//            for (PlanLocalPolitica pl: listaPoliticas) {
//                //planSistemaService.create(l);
//                System.out.println(pl.getDescripcion());
//            }
//            for (PlanLocalFormat pl: listModelo) {
//                if (pl.getCodSis() != codSistema) {
//                    
//                    for (PlanLocalFormat pl2: listModelo) {
//                        while (pl2.getCodPol() == codPolitica) {
//                        
//                            codPolitica = pl.getCodPol();
//                        }
//                    }
//                    
//                } else {
//                    codSistema = pl.getCodSis();
//                }
//            }
            for (PlanLocalFormat pl : listModelo) {
                //System.out.println(pl.getCodSis());
                if (pl.getCodSis() == codSistema) {
                    int pos = 0;
                    for (PlanLocalFormat pl2 : listModelo) {
                        if (pl.getCodObj() == pl2.getCodObj()) {
                            int pos2 = 0;
                            for (PlanLocalFormat pl3 : listModelo) {
                                //if (pl.getCodPol() != codPolitica) {
                                PlanLocalPolitica politica = new PlanLocalPolitica();
                                politica.setCodigo(pl3.getCodPol());
                                politica.setDescripcion(pl3.getDscPolitica());
                                politica.setPeriodo(Utils.getAnio(new Date()).shortValue());
                                politica.setEstado(true);
                                politica.setFechaVigencia(new Date());
                                politica.setFechaCaducidad(new Date());
                            ...
                            objetivo = new PlanLocalObjetivo();
                            objetivo.setCodigo(codObjetivo);
                            objetivo.setDescripcion(pl.getDescObjetivo());
                            objetivo.setPeriodo(Utils.getAnio(new Date()).shortValue());
                            objetivo.setEstado(true);
                            objetivo.setFechaVigencia(new Date());
                            objetivo.setFechaCaducidad(new Date());
                            ...
                            codObjetivo = pl.getCodObj();
                            listaObjetivos.add(objetivo);
                        }
                        pos++;
                    }
                    sistema.setCodigo(codSistema);
                    sistema.setDescripcion(pl.getDscSistema());
                    sistema.setPeriodo(Utils.getAnio(new Date()).shortValue());
                    sistema.setEstado(true);
                    sistema.setFechaVigencia(new Date());
                    sistema.setFechaCaducidad(new Date());
                    sistema.setPlanLocalObjetivoList(listaObjetivos);
                    //planSistemaService.create(sistema);
                } else {
                    codSistema = pl.getCodSis();
                    listaSistemas.add(sistema);
                }
