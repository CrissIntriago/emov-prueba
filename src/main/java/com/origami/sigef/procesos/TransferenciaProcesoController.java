 ...
 * @author OrigamiEC
 */
@Named(value = "transferenciaProcesoView")
@ViewScoped
public class TransferenciaProcesoController extends BpmnBaseRoot implements Serializable {
    ...
    public void guardarDatos() {
        List<DetalleTransferencias> temp = detalleTransaccionService.getSinAcreditar(transferencias);
        progress2 = 0;
        ...
        try {
            switch (getTramite().getTipoTramite().getAbreviatura()) {
                case "PAGORMU":
                    ...
                    getParamts().put("usuarioAutPago", clienteService.getrolsUser(RolUsuario.directorFinanciero));
                    break;
    ...
    public void completarTarea(Boolean accion) {
        if (accion) {
            if (getTramite().getTipoTramite().getAbreviatura().equals("PAG_DEC")) {
                getParamts().put("aprobado", 1);
            }
            if (getTramite().getTipoTramite().getAbreviatura().equals("PPRM")) {
