 ...
 */
@Named(value = "planAnualView")
@ViewScoped
public class PlanAnualPoliticaPublicaController extends BpmnBaseRoot implements Serializable {
...
//</editor-fold>
    ...
    private CupoPresupuesto cupoPresupuesto;
    ...
    public void init() {
        try {
            if (this.session.getTaskID() != null) {
                ...
                cupoPresupuesto = new CupoPresupuesto();
                if ("realizar_papp".equals(tarea.getTaskDefinitionKey())) {
                    cupoPresupuesto = cupoPresupuestoService.obtenendoCupo(BigInteger.valueOf(tramite.getNumTramite()), session.getNameUser());
                }
    ...
    public void cargandofecha() {
        ...
        Distributivo d = clienteService.getuusuarioLogeado(session.getNameUser());
...
//        planAnualProgramaLazy.getFilterss().put("estadoPapp.codigo", Arrays.asList("RP", "AP", "REP"));
        ...
        // productoLazy.getFilterss().put("actividadOperativa.unidadResponsable:equal", cupoPresupuesto.getUnidadAdministrativa());
        ...
        // lazyActividadOperativas.getFilterss().put("unidadResponsable:equal", cupoPresupuesto.getUnidadAdministrativa());
        ...
        if (busqueda.getAnio() == 0) {
            ...
            if (!edit) {
                if (politicaLocalSeleccionado != null && politicaNacionalSeleccionada != null && planLocalProgramaProyecto.getDescripcion() != "") {
                    ...
                    planLocalProgramaProyecto.setUsuarioCreacion(session.getNameUser());
                    planLocalProgramaProyecto.setUsuarioModifica(session.getNameUser());
                    ...
                    planAnual.setUsuarioModifica(session.getNameUser());
    ...
    public void seModificoPPPA(Boolean modifico, Short periodo) {
        if (modifico) {
            ...
            for (ProformaPDTO ob : lista) {
                ...
                proformaPresupuesto.setUsuarioCreacion(session.getNameUser());
                proformaPresupuesto.setFechaCreacion(new Date());
                proformaPresupuesto.setUsuarioModificacion(session.getNameUser());
...
//        return findByNamedQuery("CuentaContable.findByPadre", new Object[]{padre.getId()});
    ...
    public void savePlanNombreProyecto() {
        if (programaProyecto.getNombreProgramaProyecto() == null) {
            ...
            if (!edit) {
                ...
                programaProyecto.setUsuarioCreacion(session.getNameUser());
                programaProyecto.setUsuarioModifica(session.getNameUser());
    ...
    public void añadirPlanAnualProgramaProyecto(PlanAnualProgramaProyecto p) {
        ...
        actividad.setUnidadResponsable(cupoPresupuesto.getUnidadAdministrativa());
    ...
    public void saveActividadOperativa() {
        ...
        if (actividad.getId() != null) {
            ActividadOperativa actividadAnterior = service.find(ActividadOperativa.class, actividad.getId());
            cupoAsigando = cupoAsigando.setScale(2, RoundingMode.HALF_UP).subtract(actividadAnterior.getMonto().setScale(2, RoundingMode.HALF_UP));
        }

        if (cupoAsigando.setScale(2, RoundingMode.HALF_UP).add(actividad.getMonto()).compareTo(totalCupo.setScale(2, RoundingMode.HALF_UP)) == 1) {
            JsfUtil.addWarningMessage("Aviso", "NO TIEN SUFICIENTE CUPO PARA ASIGNAR EL MONTO INDICADO");
            return;
        }
...
//        System.out.println("datas " + busqueda.getAnio() + " " + d.getUnidadAdministrativa());
    }
...
//</editor-fold>
    ...
    public void completarTarea() {
        try {
            ...
            getParamts().put("userplanificacion", clienteService.getrolsUser(RolUsuario.planificacion));//PLANIFICACION 6
            //clienteService.getUnidadUserCodigo("THU", "14")
            getParamts().put("usuariotthh", clienteService.getrolsUser(RolUsuario.titularTH)); //TALENTO HUMANO
            if (saveTramite() == null) {
                return;
            }
...
//<editor-fold defaultstate="collapsed" desc="Getters-n-Setters">
    ...
    public CupoPresupuesto getCupoPresupuesto() {
        return cupoPresupuesto;
    }

    public void setCupoPresupuesto(CupoPresupuesto cupoPresupuesto) {
        this.cupoPresupuesto = cupoPresupuesto;
    }

//</editor-fold>
    public Short getPeriodoSacado() {
        return periodoSacado;
    }

    public void setPeriodoSacado(Short periodoSacado) {
        this.periodoSacado = periodoSacado;
    }
