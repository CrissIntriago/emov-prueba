...
@Named(value = "pemView")
@ViewScoped
public class ProgramacionEgresoController extends BpmnBaseRoot implements Serializable {
    ...
    public void distribucion(ProgramacionIngresoEgreso item) {
        if (item.getActividad().equals(false)) {
            if (item.getDistribucion().equals(true)) {
                BigDecimal aux = (item.getMontoActividad().divide(meses, 2, RoundingMode.HALF_UP));
...
//                BigDecimal total = getSumaCuatrimestres(item);
//
...
//            clienteService.getUnidadUserCodigo("CPR");
            getParamts().put("usuario_financiero", clienteService.getrolsUser(RolUsuario.financiero));
            if (saveTramite() == null) {
                return;
            }
    ...
    public void reportePEG(String formato) {
        if (tipoReporte == null) {
            JsfUtil.addWarningMessage("Advertencia", "Seleccione el tipo de Reporte.");
            return;
        }
        if (tipoReporte) {
