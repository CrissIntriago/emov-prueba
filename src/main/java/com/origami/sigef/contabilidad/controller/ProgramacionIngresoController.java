...
import com.origami.sigef.contabilidad.lazy.ProgramacionIngresoEgresoLazy;
...
@Named(value = "pimView")
@ViewScoped
public class ProgramacionIngresoController extends BpmnBaseRoot implements Serializable {
...
//              lazyIngreso.getFilterss().put("estadoA.codigo", "REG");
    ...
    public void updateEstado(OpcionBusqueda p) {
        estadoGeneral = new CatalogoItem();
        estadoGeneral = pimService.getestado(p.getAnio());
    }
...
//            clienteService.getUnidadUserCodigo("CPR")
            getParamts().put("usuario_presupuesto", clienteService.getrolsUser(RolUsuario.presupuesto));
            if (saveTramite() == null) {
                return;
            }
