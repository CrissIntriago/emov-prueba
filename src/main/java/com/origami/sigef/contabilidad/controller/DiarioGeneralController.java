 ...
 */
@Named(value = "diarioGeneralView")
@ViewScoped
public class DiarioGeneralController implements Serializable {
    ...
    private Boolean booleanFacturas;
    ...
    public void initialize() {
        ...
        if (listaPeriodo != null) {
            ...
            if (!listaPeriodo.isEmpty() && indice == 1) {
                periodo = listaPeriodo.get(0).getAnio();
            } else {
                periodo = listaPeriodo.get(indice - 1).getAnio();
            }
        } else {
    ...
    public void form(DiarioGeneral diarioGeneralIntegrado, String accion) {
        if (periodo != null) {
            if (periodo.equals(opcionBusqueda.getAnio()) || accion.equals("visualizar")) {
            ...
            switch (moduloDeEnlaceSeleccionado.getCodigo()) {
                ...
                case "modulo_certificaciones_presupuestarias":
                    ...
                    try {
                        ...
                        reservaCompromisoList = diarioGeneralService.getListadoReservaCompromiso("APRO", opcionBusqueda.getAnio(), c.getTime());
...
//                if (subEnlaceSeleccionado.getCodigo().equals("ordenes_cobro")) {
//                    ordernarCuentasArriendo(arriendo.getIdArrendamiento().getLocal().getIdTarifa().getCuentaCobro(), arriendo.getMontoPagar().subtract(arriendo.getIdArrendatario().getIva()), true, null, aux, aux_1);
//                    ordernarCuentasArriendo(arriendo.getIdArrendamiento().getLocal().getIdTarifa().getCuentaIvaIngreso(), arriendo.getIdArrendatario().getIva(), true, null, aux, aux_1);
...
//                } else {
//                    ordernarCuentasArriendo(arriendo.getIdArrendamiento().getLocal().getIdTarifa().getCuentaCobro(), arriendo.getMontoPagar().subtract(arriendo.getIdArrendatario().getIva()), false, arriendo.getIdArrendamiento().getLocal().getIdTarifa().getItemPresupuesto(), aux, aux_1);
//                    ordernarCuentasArriendo(arriendo.getIdArrendamiento().getLocal().getIdTarifa().getCuentaIvaEgreso(), arriendo.getIdArrendatario().getIva(), false, null, aux, aux_1);
...
//</editor-fold>
    ...
    private void cargarValoresProcesoImpuestoSRI() {
        if (!servletSession.estaVacio()) {
            ...
            if (servletSession.getParametros().containsKey("tramite")) {
