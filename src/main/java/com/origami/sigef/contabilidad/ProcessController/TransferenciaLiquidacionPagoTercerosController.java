 ...
 * @author Luis Suarez
 */
@Named(value = "liquidacionFondoTrnasfeView")
@ViewScoped
public class TransferenciaLiquidacionPagoTercerosController extends BpmnBaseRoot implements Serializable {
...
//                System.out.println("Abreviatura " + getTramite().getTipoTramite().getAbreviatura());
    ...
    public void addArchivo() {
        ...
        switch (getTramite().getTipoTramite().getAbreviatura()) {
            ...
            case "proceso_af_caja_chica":
                ...
                break;
            default:
                Cliente cx = comprobantePagoService.getClientecomprobantePago(BigInteger.valueOf(tramite.getNumTramite()));
                asunto = tramite.getTipoTramite().getDescripcion();
                mensaje = "<html lang=\"es\">\n"
                        + "<head>\n"
                        + "<meta charset=\"utf-8\"/>\n"
                        + "</head>\n"
                        + "<body>\n"
                        + "<p style=\"width:200px;\">SR(a). " + cx.getNombreCompleto()
                        + "\n POR MEDIO DE LA PRESENTE SE LE NOTIFICA QUE LA " + tramite.getTipoTramite().getDescripcion() + " HA SIDO APROBADA"
                        + " SEGÚN EL NUMERO DE TRÁMITE N° " + comprobanteP.getNumeroTramite() + " </p>\n"
                        + "</body>\n"
                        + "</html>";
                getParamts().put("aprobado", 1);
                notificar(asunto, cx.getEmail(), mensaje, cx.getNombreCompleto());
                break;

            case "proceso_liquidacion_pterceros":
                Cliente c1 = comprobantePagoService.getClientecomprobantePago(BigInteger.valueOf(tramite.getNumTramite()));
                asunto = tramite.getTipoTramite().getDescripcion();
                mensaje = "<html lang=\"es\">\n"
                        + "<head>\n"
                        + "<meta charset=\"utf-8\"/>\n"
                        + "</head>\n"
                        + "<body>\n"
                        + "<p style=\"width:200px;\">SR(a). " + c1.getNombreCompleto()
                        + "\n POR MEDIO DE LA PRESENTE SE LE NOTIFICA QUE LA " + tramite.getTipoTramite().getDescripcion() + " HA SIDO APROBADA"
                        + " SEGÚN EL NUMERO DE TRÁMITE N° " + comprobanteP.getNumeroTramite() + " </p>\n"
                        + "</body>\n"
                        + "</html>";
                getParamts().put("aprobadoTe", 1);
                notificar(asunto, c1.getEmail(), mensaje, c1.getNombreCompleto());
                break;
