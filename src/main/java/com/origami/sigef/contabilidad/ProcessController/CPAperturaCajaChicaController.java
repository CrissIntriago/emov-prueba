 ...
 * @author Luis Suarez
 */
@Named(value = "cpApeturaCajaChicaView")
@ViewScoped
public class CPAperturaCajaChicaController extends BpmnBaseRoot implements Serializable {
    ...
    public void deleteComprobantepago(int c) {
        detalleComprobantePagoList.remove(c);
        deleteActualizarValores();
    }
...
//            this.subEnlaceSeleccionado = catalogoItemService.getPadreCatalogoItem(enlaceSeleccionado);
            if (getTramite().getTipoTramite().getAbreviatura().equals("RET_JUD")) {
    ...
    public void completarTarea() {
        ...
        try {
            ...
            PrimeFaces.current().ajax().update(":frmDlgObser");
            //saveComprobante();
            getParamts().put("usuarioTransferencia", clienteService.getrolsUser(RolUsuario.tesorero)); //TESORERO 5
    ...
    public void deleteActualizarValores() {
        actualizarTotales();
        actualizarTotalBeneficiario();
