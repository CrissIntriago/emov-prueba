 ...
 * @author Luis Suarez
 */
@Named(value = "liquidacionCCRcView")
@ViewScoped
public class LiquidacionCChicaRegistroContableController extends BpmnBaseRoot implements Serializable {
    ...
    private Boolean esconderAccionAfavor;
    private boolean btnEnlace;
    private boolean btnSiguienteTarea;
    private boolean btnAprobar, btnRechazar, btnAfavor;
    private boolean validateSubidaDoc = false;
            ...
            if (!JsfUtil.isAjaxRequest()) {
                ...
                switch (getTramite().getTipoTramite().getAbreviatura()) {
                    case "proceso_jgvis_servicios":
                        esconderAccionAfavor = Boolean.TRUE;
                        break;
                    case "CIERRE_CAJA":
                        esconderAccionAfavor = Boolean.FALSE;
                        validateSubidaDoc = true;
                        btnCompletarTarea = true;
                        break;
                    default:
                        esconderAccionAfavor = Boolean.FALSE;
...
//</editor-fold>
                ...
                case 0:
                    switch (getTramite().getTipoTramite().getAbreviatura()) {
                        ...
                        case "CIERRE_CAJA":
                            ...
                            break;
                        default:
                            getParamts().put("aprobadoCero", a);
                            getParamts().put("usuarioTesoreria", clienteService.getrolsUser(RolUsuario.tesorero)); ///Tesoreria 5
                            break;
                    }
                    break;
                case 1:
                ...
                case 2:
                    switch (getTramite().getTipoTramite().getAbreviatura()) {
                        case "proceso_jgvis_servicios":
                            ...
                            break;
                        default:
                            getParamts().put("aprobadoContabilidad", 0);
                            getParamts().put("usuarioAutorizacionPago", clienteService.getrolsUser(RolUsuario.directorFinanciero));
                            break;
                    }
            }
            if (saveTramite() == null) {
    ...
    public void completarTarea() {
        try {
            ...
            getParamts().put("usuarioVerificacionContable", clienteService.getrolsUser(RolUsuario.contador)); //CONTADOR 3
            if (saveTramite() == null) {
                return;
            }
            JsfUtil.executeJS("PF('dlgObservaciones').hide()");
...
//</editor-fold>
    ...
    public boolean isValidateSubidaDoc() {
        return validateSubidaDoc;
    }

    public void setValidateSubidaDoc(boolean validateSubidaDoc) {
        this.validateSubidaDoc = validateSubidaDoc;
    }
    ...
    public Boolean getEsconderAccionAfavor() {
        return esconderAccionAfavor;
    }

    public void setEsconderAccionAfavor(Boolean esconderAccionAfavor) {
        this.esconderAccionAfavor = esconderAccionAfavor;
    }

    public boolean isBtnAfavor() {
        return btnAfavor;
    }
