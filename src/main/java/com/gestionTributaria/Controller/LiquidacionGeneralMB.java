 ...
 */
@Named(value = "liquidacionGenTributarioView")
@ViewScoped
public class LiquidacionGeneralMB extends ReportGenealModel implements Serializable {
    ...
    @Inject
    private FinaRenDetalleLiquidacionService liquidacionDetService;
...
//    protected RenLiquidacionesLazy titulosCreditoPagado;
    ...
    private Long numLiquiSaldo = 0L;

    public Long getNumLiquiSaldo() {
        return numLiquiSaldo;
    }

    public void setNumLiquiSaldo(Long numLiquiSaldo) {
        this.numLiquiSaldo = numLiquiSaldo;
    }
    private BigDecimal saldoLiqui = BigDecimal.ZERO;
    private Boolean rImput;
    private Boolean descuentoSaldo = Boolean.TRUE;
    private List<FinaRenDetLiquidacion> listaSaldo;
    private List<FinaRenLiquidacion> listaLlenarCampos;
    ...
    public void initView() {
        ...
        listaLlenarCampos = new ArrayList<>();
        try {
            ...
            // tipoReporteLiquiList = manager.findAllQuery("select distinct rl.tipoLiquidacion from FinaRenLiquidacion rl order by rl.tipoLiquidacion.nombreTitulo", null);

            if (service.isAdmin(userSession.getNameUser(), "admin") || service.isAdmin(userSession.getNameUser(), "JEFA DE RENTAS")) {
                llenarArbolAdmin();
            } else {
                llenarArbol();
            }
    ...
    public BigDecimal liquisSaldoTerreno(Long num) {
        FinaRenDetLiquidacion t = null;
        FinaRenDetLiquidacion bandera = null;
        BigDecimal x = BigDecimal.ZERO;
        t = liquidacionDetService.liquisSaldoTerreno(num);
//        System.out.println("el valor del nuevo modo es --_>> "+t.getValor());
        if (t != null) {
            if (listaSaldo == null && listaSaldo.isEmpty()) {
                t.getLiquidacion().setTotalPago(t.getValor());
                listaLlenarCampos.add(t.getLiquidacion());

                listaSaldo.add(t);
                JsfUtil.update("mainForm:tvRecaudaciones:panelDetalle");
            } else {
                if (!listaSaldo.contains(t)) {
                    t.getLiquidacion().setTotalPago(t.getValor());
                    listaLlenarCampos.add(t.getLiquidacion());
                    //listaLlenarCampos.add(t.getLiquidacion());
                    listaSaldo.add(t);
        ...
        return gettotalTerreno();
    }

    public BigDecimal gettotalTerreno() {
        BigDecimal suma = BigDecimal.ZERO;
        for (FinaRenDetLiquidacion item : listaSaldo) {
            System.out.println("listaSaldoValor--->>  " + item.getValor());
            suma = suma.add(item.getValor());
        }
    ...
    public void llenarSaldo() {
        ...
        saldoLiqui = BigDecimal.ZERO;

        try {
//            if (numLiquiSaldo != null && !numLiquiSaldo.isEmpty()) {
            if (numLiquiSaldo != null && numLiquiSaldo>0L) {
                tt = liquisSaldoTerreno(numLiquiSaldo);
                System.out.println("la prueba de la nueva busqueda es --->>>"+tt);
                System.out.println("LA LISTA ES---->" + listaSaldo);
                if (tt != BigDecimal.ZERO) {
                    setSaldoLiqui(tt);
                    JsfUtil.addInformationMessage("ATENCION", "Saldo encontrado para la liquidacion: " + numLiquiSaldo + " de: " + getSaldoLiqui());
                    JsfUtil.update("mainForm:tvRecaudaciones:panelDetalle");
                } else {
                    JsfUtil.addWarningMessage("ATENCION", "No se encontro ningun saldo con el Numero de liquidacion: " + numLiquiSaldo);
                    return;
                }
            } else {
                JsfUtil.addWarningMessage("ATENCION", "Ingrese una liquidacion para buscar");
                return;
            }
            numLiquiSaldo = 0L;
        } catch (Exception e) {
    ...
    public void setearSaldo() {
        System.out.println("saldo de laliqui---->> " + getSaldoLiqui());
        System.out.println("saldo de laliqui 2----->  " + saldoLiqui);
        descuentoSaldo = Boolean.TRUE;
        if (saldoLiqui != null) {
            descuentoSaldo = Boolean.FALSE;
            for (FinaRenRubrosLiquidacion item : rubrosSeleccionado) {
                if (item.getId().equals(584L)) {
                    System.out.println("valor del item" + item.getValorTotal());
                    item.setValorTotal((item.getValorTotal().subtract(getSaldoLiqui())));
                    item.setValor(item.getValorTotal());
//           rubroSelect.setValorTotal((rubroSelect.getValor().multiply(new BigDecimal(rubroSelect.getCantidad()))).subtract(saldoLiqui));
                }
                System.out.println("valor del item" + item.getValorTotal());
            }
        ...
        System.out.println("descuentoSaldo--->>>  " + descuentoSaldo);
    }
    ...
    public void llenarArbolAdmin() {
        ...
        raices = manager.findAllQuery("SELECT r FROM FinaRenTipoLiquidacion r WHERE r.transaccionPadre = :idPadre AND r.nombreTransaccion IS NOT NULL AND r.mostrarTransaccion = true and r.estado = true and r.especieValorada = FALSE"
    ...
    public void buscarCliente() {
        try {
            if (ente2.getIdentificacion() != null) {
                ...
                Cliente cli = this.manager.findByParameter(Cliente.class, paramcli);
...
//        if (liquidacion.getTipoLiquidacion().equals(183L)) {
//            abrirReporteTerreno(liquidacion);
//        }
        initView();
...
//            aniosPrivado = "";
    ...
    public Boolean selectintp() {
        rImput = Boolean.FALSE;
        if (cobrosGenerales.getTipoLiquidacion().getId() == 183L) {
            setrImput(Boolean.TRUE);
            System.out.println("dfsdff--->>  " + getrImput());
            JsfUtil.update("mainForm:tvRecaudaciones:panelDetalle");
        } else {
            setrImput(Boolean.FALSE);
            System.out.println("dfsdff--->>  " + getrImput());
            JsfUtil.update("mainForm:tvRecaudaciones:panelDetalle");
        }
        return rImput;
    ...
    public void onNodeSelect() {
        ...
        if (selectedNode != null) {
            ...
            if (cobrosGenerales.getTipoLiquidacion() != null) {
                parametros = new HashMap<>();
                parametros.put("tipoLiquidacion", cobrosGenerales.getTipoLiquidacion());
                result = manager.findAll(FinaRenRubrosLiquidacion.class, parametros);
                if (result != null) {
                    rubrosSeleccionado = new ArrayList<>();
                    for (FinaRenRubrosLiquidacion item : result) {
                        item.setCantidad(1);
                        item.setCobrar(Boolean.FALSE);
                        if (item.getValor() == null) {
                            item.setValor(BigDecimal.ZERO);
                        }
                        item.setValorTotal(new BigDecimal(item.getCantidad().toString()).multiply(item.getValor()).setScale(2, RoundingMode.HALF_UP));
                        rubrosSeleccionado.add(item);
                    }
                }
            }
            ...
            System.out.println("tipo id liquidacion--->  " + cobrosGenerales.getTipoLiquidacion().getId());
    ...
    public void editarDetallePago(FinaRenRubrosLiquidacion item, int index) {
        if (item.getCantidad() != null && item.getValor() != null) {
            item.setValorTotal(new BigDecimal(item.getCantidad().toString()).multiply(item.getValor()).setScale(2, RoundingMode.HALF_UP));
        }
        rubrosSeleccionado.remove(index);
        rubrosSeleccionado.add(index, item);
        calcularTotalCobroGeneral();
    ...
    public void seleccionarLiquidacion() {
        ...
        if (predio != null && predio.getId() != null) {
            ...
            if (cobrosGenerales.getTipoLiquidacion().getPredio().equals("TER")) {
                JsfUtil.addWarningMessage("", "Seleccion un predio");
                return;
...
//    public void abrirReporteTerreno(FinaRenLiquidacion liq) {
//        try {
//           
//            String path = FacesContext.getCurrentInstance().getExternalContext().getRealPath("/");
//            ss.instanciarParametros();
//            boolean impresion = false;
//            ss.addParametro("ALGO", liq.getPredio());
//            ss.setNombreReporte("sReporteVentaTerreno");
//            tipoReporteLiqui = new FinaRenTipoLiquidacion();
//
//                        ss.addParametro("impresion", impresion);
//
...
//    }
    ...
    public void generarLiquidacion() {
        if (session != null && session.getNameUser() != null) {
            ...
            if (listaSaldo != null && !listaSaldo.isEmpty()) {
                for (FinaRenDetLiquidacion liq : listaSaldo) {

                    liq.getLiquidacion().setCodigoVerificado(new Boolean("true"));
                    // liq.getLiquidacion().get
                    liq.getLiquidacion().setLiquidacionTerreno(liquidacion.getId());

                    liquidacionService.edit(liq.getLiquidacion());
    ...
    public void aceptarSaldo() {
        if (saldoLiqui.compareTo(BigDecimal.ZERO) > 0) {

            if (rubroSelect.getTipoValor().getId() == 584L) {
                System.out.println("volor rubro>>" + rubroSelect.getValor() + "  cantidad >>" + rubroSelect.getCantidad());
    ...
    public void calcularTotalCobroGeneral() {
        ...
        if (this.rubrosSeleccionado != null && !this.rubrosSeleccionado.isEmpty()) {
            for (FinaRenRubrosLiquidacion r : rubrosSeleccionado) {
                if (r.getCobrar() != null && r.getCobrar()) {
            ...
            switch (this.tipoReporte2.intValue()) {
                ...
                case 1:
                    ...
                    if (fechaCierre != null) {
                        ...
                        ss.addParametro("FECHA_CIERRE", new Date());
                        impresion = true;
                    }
                ...
                case 5:
                    ...
                    break;
                case 6:
                    ss.setNombreReporte("sdesgloseRubrosRentas");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        impresion = true;
                    } else {
                    ...
                    break;
                case 7:
                    ss.setNombreReporte("sdesgloseContribuyentesRentas");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
                    } else {
                        ss.addParametro("FECHA_CIERRE", new Date());
                        impresion = false;
                    ...
                    break;
                case 8:
                    ss.setNombreReporte("sdesgloseContribuyentesRentasEmision");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
                    ...
                    break;
                case 9:
                    ss.setNombreReporte("urbanoEmision");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
                ...
                case 11:
                    ss.setNombreReporte("sReporteCarteraVencidaCoactiva");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
                    ...
                    break;
                case 12:
                    ss.setNombreReporte("sReporteRubrosEmision");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
                    ...
                    break;
                case 13:
                    ss.setNombreReporte("sDesgloecAlcabalaPlusvalia");
                    if (fechaCierre != null) {
                        ss.addParametro("FECHA_CIERRE", fechaCierre);
                        //ss.addParametro("SUBREPORT_DIR", "GestionTributatia/Transacciones");
                        impresion = true;
...
//                    break;
                ...
                case 100:
                    List<FinaRenLiquidacion> liquidacionTemp = liquidacionService.listaLiquidacionTmp(numLiquidacionReport);
                    ...
                    if (liquidacionTemp != null) {
                        ...
                        if (tmp.getComprador() != null) {
                            cliente = tmp.getComprador().getNombreCompleto();
                            identificacion = tmp.getComprador().getIdentificacionCompleta();
                        } else {
                            cliente = tmp.getNombreComprador();
                            identificacion = tmp.getIdentificacion();
                        }
                        liquidadorResponsable = tmp.getLiquidadorResponsable();
                        descripcion = "El Departamento de Rentas del Gobierno Autónomo Descentralizado Municipal del Cantón Durán le envia a Ustedes los valores que su empres con código "
                                + tmp.getPredio().getClaveCat() + "-" + tmp.getLocalComercial().getNumLocal() + " e Identificación " + identificacion + " adeudada por los siguiente conceptos:";
                    }
                    ss.addParametro("cliente", cliente);
                    ss.addParametro("num_liquidacion", numLiquidacionReport);
                    ss.addParametro("liquidadorResponsable", liquidadorResponsable);
                    ss.addParametro("usuario_impresion", userSession.getNameUser());
                ...
                default:
                    ...
                    if (tipoReporteLiqui != null && tipoReporteLiqui.getId() != null) {
                        ss.setNombreReporte("detalleRubrosRecaudadosTipoTitulos");
                        // ss.setNombreSubCarpeta("manageres");
                        ss.addParametro("TIPO", tipoReporteLiqui.getId());
                        System.out.println("TIPO LIQUIDACION" + tipoReporteLiqui.getId());
                    } else {
                        return;
                    }
                    break;
...
//        }
    ...
    *CALCULO DE REMISION DE INTERESES
    ...
    public List<FinaRenLiquidacion> getListaLlenarCampos() {
        return listaLlenarCampos;
    }

    public void setListaLlenarCampos(List<FinaRenLiquidacion> listaLlenarCampos) {
        this.listaLlenarCampos = listaLlenarCampos;
    }

    public List getListaSaldo() {
        return listaSaldo;
    }

    public void setListaSaldo(List listaSaldo) {
        this.listaSaldo = listaSaldo;
    }

    public Boolean getDescuentoSaldo() {
        return descuentoSaldo;
    }

    public void setDescuentoSaldo(Boolean descuentoSaldo) {
        this.descuentoSaldo = descuentoSaldo;
    }

    public Boolean getrImput() {
        return rImput;
    }

    public void setrImput(Boolean rImput) {
        this.rImput = rImput;
    }

    public BigDecimal getSaldoLiqui() {
        return saldoLiqui;
    }

    public void setSaldoLiqui(BigDecimal saldoLiqui) {
        this.saldoLiqui = saldoLiqui;
    }
