...
import com.gestionTributaria.Services.AppDepartamentoDetalleTitulosServices;
 ...
 */
@Named(value = "planificacionView")
@ViewScoped
public class PlanificacionMB extends ReportGenealModel implements Serializable {
    ...
    @Inject
    private AppDepartamentoDetalleTitulosServices appDepartamentoDetalleTitulosServices;
...
//    private ConvenioMedidorLazy convenioMedidorLazy;
    ...
    private List<UnidadAdministrativa> departamentosIds;
    ...
    public void initView() {
        ...
        try {
            if (session != null) {
                ...
                deps = unidadAdministrativaService.hijasUnidadesRecursivo(session.getUsuario());
                tipoCertificado = Boolean.FALSE;
                //catPlanificacionTitulosLazy = new CatPlanificacionTitulosLazy();               
                esLiq = true;
                titulosReporte.clearCache();
                //root2 = titulosReporte.getTreePlanificacion();
                paramt = new HashMap<>();
        ...
        try {
                ...
                if (titulosCredito == null) {
                  ...
                  //  titulosCredito = new LazyModel<>(FinaRenLiquidacion.class);
                    JsfUtil.update("mainForm:tvRecaudaciones:dtLiquidaciones");
                        ...
                        liquidacionesLazy.getFilterss().put("tipoLiquidacion.transaccionPadre", 180);                       
            ...
            if (event.getTab().getId().equals("tituloCreditoPagado")) {
                ...
                if (titulosCreditoPagado == null) {
                    ...
                    titulosCreditoPagado.getFilterss().put("tipoLiquidacion.prefijo", "CMO");
...
//            catPredioLazy = new CatPredioLazy();
    ...
    public AppDepartamentoDetalleTitulos guardarDeatallePlanificacion() {
        AppDepartamentoDetalleTitulos detalle = new AppDepartamentoDetalleTitulos();
        detalle.setCementerioBoveda(boveda);
        detalle.setDepartamento(session.getDepartamento());
        detalle.setEnte(cobrosGenerales.getComprador());
        detalle.setFecha(new Date());
        detalle.setTipoLiquidacion(cobrosGenerales.getTipoLiquidacion());

        if (detalle.getId() != null) {
            appDepartamentoDetalleTitulosServices.edit(detalle);
        } else {

            detalle = appDepartamentoDetalleTitulosServices.create(detalle);
        }
...
//            }
            ...
            cobrosGenerales.setDepartamentoDetalleTitulos(guardarDeatallePlanificacion());
            this.liquidacion = cobrosGenerales;
            if (this.liquidacion != null && this.liquidacion.getTipoLiquidacion() != null && this.rubrosSeleccionado != null && !this.rubrosSeleccionado.isEmpty()) {
                if ((this.liquidacion.getComprador() != null || this.liquidacion.getNombreComprador() != null) && this.liquidacion.getSaldo() != null && this.liquidacion.getSaldo().compareTo(BigDecimal.ZERO) > 0) {
                    this.liquidacion.calcularPago();
...
//        }
    ...
    public void savePlanificacionTitulo() {
        ...
        for (RenDepartamentoDetalleTitulos rpt : planificacionTitulosList) {
            RenDepartamentoDetalleTitulos data = Utils.clone(rpt);
            data.setPlanificacion(catPlanificacionTitulos);
            if (data.getId() != null) {
                data = departamentoDetalleTitulosServices.create(data);
            } else {
                departamentoDetalleTitulosServices.edit(data);
    ...
    public void llenarArbolAdmin() {
        ...
        paramt = new HashMap<>();
        paramt.put("items", Arrays.asList("CMO"));
    ...
    public void llenarArbol() {
        try {
            ...
            List<RenTipoLiquidacionDepartamento> departamentos = service.listaRubrosPadres(session.getNameUser(), Arrays.asList("CMO"));
    ...
    public void generarLiquidacion() {
        if (session != null && session.getNameUser() != null) {
            ...
            System.out.println("registro " + liquidacion.getAnio());
...
//        ss.instanciarParametros();
//
//        ss.setNombreSubCarpeta("permisos_certificados");
//        ss.setNombreReporte(liquidacion.getTipoLiquidacion().getNombreReporte());
//
