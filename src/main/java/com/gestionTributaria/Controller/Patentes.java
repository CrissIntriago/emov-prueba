...
import com.gestionTributaria.Services.HiLocalActividadesServices;
 ...
 */
@Named(value = "patenteView")
@ViewScoped
public class Patentes implements Serializable {

    private static final Logger LOG = Logger.getLogger(Patentes.class.getName());
    ...
    @Inject
    private HiLocalActividadesServices hiLocalActividadesServices;
    ...
    private HiLocalActividades historialActividades;
    private LazyModel<HiLocalActividades> lazyHiActividades;
    private List<FinaRenActividadComercial> actividadList;

    @PostConstruct
    public void init() {
        historialActividades = new HiLocalActividades();
...
//                System.out.println("idPatente " + ss.getParametros().get("idPatente"));
//                idPatente = Long.valueOf(ss.getParametros().get("idPatente").toString());
//                patentes = new LazyModel<>(FinaRenPatente.class);
//                patentes.addFilter("id", idPatente);
//                patentes.getSorteds().put("id", "ASC");
//                patentes.setDistinct(false);
...
//        }        
    ...
    public void historiLocaes(FinaRenLocalComercial local) {
        this.local = new FinaRenLocalComercial();
        this.local = local;
        lazyHiActividades = new LazyModel(HiLocalActividades.class);
        lazyHiActividades.getFilterss().put("local", local);

    }

    public void editHistorialActividades(HiLocalActividades hiLocal) {

        param = new HashMap<>();
        param.put("estado", true);
        actividadList = new ArrayList<>();
        actividadList = services.findAll(FinaRenActividadComercial.class, param);

        if (hiLocal != null) {
            historialActividades = hiLocal;
        } else {
            historialActividades = new HiLocalActividades();
        }
    ...
    public void saveUpdateHiActividades() {

        if (historialActividades.getId() != null) {
            hiLocalActividadesServices.edit(historialActividades);
        } else {
            historialActividades.setLocal(this.local);
            historialActividades = hiLocalActividadesServices.create(historialActividades);
        }
        historialActividades = new HiLocalActividades();
        JsfUtil.addInformationMessage("", "Transacción exitoso");
    }
    ...
    public void openMemo(FinaRenLocalComercial lo) {
        ...
        descrpcioFinal = "Adjunto: Documentación de soporte del Memoraándum No. GADMCD-CTM-" + Utils.getAnio(new Date()).toString() + "-" + numMemorandActual + "-M";
    ...
    public void saveSecuencia() {
        SecuenciaMemo veri = new SecuenciaMemo();
        param = new HashMap<>();
        param.put("anio", anioMan);
        param.put("codigo", "DGJVVCACM");
        veri = services.findByParameter(SecuenciaMemo.class, param);
        if (veri.getId() != null) {
            veri.setNum(mantenimientoSecuencia.getNum());
            secuenciaMemoService.edit(veri);
        } else {
            mantenimientoSecuencia.setCodigo("DGJVVCACM");
            mantenimientoSecuencia.setAnio(anioMan.shortValue());
            secuenciaMemoService.create(mantenimientoSecuencia);
        }
        mantenimientoSecuencia = new SecuenciaMemo();
        JsfUtil.executeJS("PF('dlogoManSecuen').hide()");
        JsfUtil.update("frmManSecuencia");
    }
...
//        enteListLazy = new ClienteLazy();
//        predioListLazy = new CatPredioLazy("A");
//        ubicaciones = new LazyModel<>(RenLocalUbicacion.class, "descripcion", "ASC");
...
//            if (services.guardarLocalComercial(localComercial, activosLocal) != null) {
//
...
//<editor-fold defaultstate="collapsed" desc="SETTER AND GETTER">
    ...
    public FinaRenLocalComercialService getComercialService() {
        return comercialService;
    }

    public void setComercialService(FinaRenLocalComercialService comercialService) {
        this.comercialService = comercialService;
    }

    public FinaRenActividadComercialServices getActividadComercialServices() {
        return actividadComercialServices;
    }

    public void setActividadComercialServices(FinaRenActividadComercialServices actividadComercialServices) {
        this.actividadComercialServices = actividadComercialServices;
    }

    public PatenteService getPatenteService() {
        return patenteService;
    }

    public void setPatenteService(PatenteService patenteService) {
        this.patenteService = patenteService;
    }

    public RenLocalComercialFotoServices getComercialFotoService() {
        return comercialFotoService;
    }

    public void setComercialFotoService(RenLocalComercialFotoServices comercialFotoService) {
        this.comercialFotoService = comercialFotoService;
    }

    public RenLocalCantidadAccesoriosServices getAccesoriosServices() {
        return accesoriosServices;
    }

    public void setAccesoriosServices(RenLocalCantidadAccesoriosServices accesoriosServices) {
        this.accesoriosServices = accesoriosServices;
    }

    public SecuenciaMemoService getSecuenciaMemoService() {
        return secuenciaMemoService;
    }

    public void setSecuenciaMemoService(SecuenciaMemoService secuenciaMemoService) {
        this.secuenciaMemoService = secuenciaMemoService;
    }
    ...
    public HiLocalActividadesServices getHiLocalActividadesServices() {
        return hiLocalActividadesServices;
    }

    public void setHiLocalActividadesServices(HiLocalActividadesServices hiLocalActividadesServices) {
        this.hiLocalActividadesServices = hiLocalActividadesServices;
    }

    public HiLocalActividades getHistorialActividades() {
        return historialActividades;
    }

    public void setHistorialActividades(HiLocalActividades historialActividades) {
        this.historialActividades = historialActividades;
    }

    public LazyModel<HiLocalActividades> getLazyHiActividades() {
        return lazyHiActividades;
    }

    public void setLazyHiActividades(LazyModel<HiLocalActividades> lazyHiActividades) {
        this.lazyHiActividades = lazyHiActividades;
    }

    public List<FinaRenActividadComercial> getActividadList() {
        return actividadList;
    }

    public void setActividadList(List<FinaRenActividadComercial> actividadList) {
