...
@Named
@ViewScoped
public class LiquidacionInquilinatoMB extends BpmnBaseRoot implements Serializable {
...
//        }
    ...
    public void liquidar(InquilinatoCarpetaDetalle inquil) {
        ...
        if (!inquil.getInquilinatoCarpeta().getEstado()) {
            JsfUtil.addWarningMessage("", "La carpeta para este predio se encuentra cerrada o inactiva");
        }
...
//            JsfUtil.addWarningMessage("", "solo de debe genera una liquidacion por trsmite.");
//            return;
//        }
...
//        System.out.println("quiero ver el estado de esta wea " + Utils.isNotEmpty(liquidacionService.liquidacionByTramite(BigInteger.valueOf(tramite.getId()), new FinaRenEstadoLiquidacion(2L))));
    ...
    public void completarTarea() {
        try {
            ...
            getParamts().put("recaudaciones", usuario.equals("") ? session.getNameUser() : usuario);
            if (saveTramite() == null) {
                return;
            }
