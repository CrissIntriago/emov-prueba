...
@Named
@ViewScoped
public class LiuqidacionViaPublicaMB extends BpmnBaseRoot implements Serializable {
    
    private static final Logger LOG = Logger.getLogger(LiuqidacionViaPublicaMB.class.getName());
    ...
    public void verificacion() {
        ...
        comisaria.setTipoPermiso(service.find(JvClasesPermisos.class, tipoPermiso.longValue()));
        System.out.println("tipoPermiso " + comisaria.getTipoPermiso());
        if (comisaria.getTipoPermiso() != null && comisaria.getTipoPermiso().getCodigoClsePermiso() != null) {
            System.out.println(comisaria.getTipoPermiso().getCodigoClsePermiso());
        }
...
//            }
        ...
        if (catalogo != null) {
            ...
            comisaria.setOrigen("VIA_PUBLICA");
            ...
            comisaria.setEnte(propietario);
            comisaria.setOrigen("VIA_PUBLICA");
            comisaria.setMulta(Boolean.FALSE);
            //  comisaria.setNumSolar(comisariaServices.numSolar());

            if (!calcularPermisoViaPublica()) {
                return;
            }
            comisaria.setObservacion(observacion.getObservacion());
            if (comisaria.getId() == null) {
    ...
    public void handleFileUpload(FileUploadEvent event) {
        try {
            ...
            JsfUtil.executeJS("PF('dlogoTitulo').hide()");
            JsfUtil.update("formTitulo");
    ...
    public boolean calcularPermisoViaPublica() {
        if (tipoLiquidacion != null && tipoLiquidacion.getId() != null) {
            ...
            JsfUtil.addWarningMessage("", "No se pudo relaizar la transacción correctamente");
            return false;
        }
    }
    ...
    public void generarLiquidacion() {
        if (session != null && session.getNameUser() != null && tipoLiquidacion != null && tipoLiquidacion.getId() != null) {
            ...
            liquidacion.setTramite(BigInteger.valueOf(this.tramite.getId()));
    ...
    public void completarTarea(int index) {
        try {
            ...
            getParamts().put("necesita_renta", index);
            String usuario = "";
            if (index == 1) {
                usuario = clienteService.getrolsUser(RolUsuario.aprobadoraRentas);
                getParamts().put("rentas", usuario);
            } else {
                usuario = clienteService.getrolsUser(RolUsuario.tesorero);
                getParamts().put("recuadaciones", usuario);
            }
